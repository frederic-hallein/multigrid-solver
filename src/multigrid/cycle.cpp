...
)
{
    ...
    // if (level == grids.size() - 1) {
    //     logger::debug("Reached coarsest grid. Directly solve for v.");
    //     grid.v = direct_solve(grid.f, h_x, h_y, sigma);
    //     return;
    // }

    // for (unsigned int iter = 1; iter <= smoother_param.nu_1; ++iter) {
    //     logger::debug("Pre-smoothing ({}/{})", iter, smoother_param.nu_1);
    //     smoother(grid.v, grid.f, h_x, h_y, sigma, omega);
    // }
    ...
    // std::fill(grids[level + 1].v.begin(), grids[level + 1].v.end(), 0.0);
    // v_cycle(grids, level + 1, 2 * h_x, 2 * h_y, sigma, omega, smoother, smoother_param);

    // logger::debug("Prolongating and correcting (level = {})", level);
    // std::vector<std::vector<double>> correction = prolongate(grids[level + 1].v);
    // for (std::size_t i = 0; i < grid.v.size(); ++i) {
    //     for (std::size_t j = 0; j < grid.v[i].size(); ++j) {
    //         grid.v[i][j] += correction[i][j];
    //     }
    // }
